...
def trusted_origin(scheme_override=None):
    """Return the origin (e.g. "https://redash.example.com") of the configured host.

    The origin is derived from the REDASH_HOST setting, which is server side
    configuration (the same setting the links in outgoing emails use, see
    ``base_url``). Returns None when REDASH_HOST isn't configured.
    """
    configured_host = (settings.HOST or "").strip().rstrip("/")
    if not configured_host:
        return None

    parts = urlsplit(configured_host)
    if not parts.netloc:
        # REDASH_HOST configured without a scheme, e.g. "redash.example.com".
        parts = urlsplit("//{}".format(configured_host))

    if not parts.netloc:
        return None

    scheme = scheme_override or parts.scheme or ("https" if settings.ENFORCE_HTTPS else "http")

    return urlunsplit((scheme, parts.netloc, "", "", ""))


def external_url_for(endpoint, _scheme=None, **values):
    """Build an absolute URL for ``endpoint`` rooted at the configured host.

    ``flask.url_for(..., _external=True)`` derives the scheme and host from the
    incoming request, i.e. from the client controlled ``Host`` (or
    ``X-Forwarded-*``) header. The absolute URLs Redash builds are handed to
    users and to third parties -- password reset / invite / verification links,
    the SAML assertion consumer service URL, the OAuth redirect URI and public
    dashboard links -- so a poisoned ``Host`` header can point them at an
    attacker controlled origin and leak the tokens they carry.

    The path is still generated by ``url_for``, only the origin comes from
    trusted configuration. When REDASH_HOST isn't configured we fall back to the
    request's own origin, so deployments that never set it keep working -- those
    deployments should set REDASH_HOST (and have their proxy reject unexpected
    ``Host`` headers) to get trustworthy absolute URLs.
    """
    values.pop("_external", None)
    _scheme = _scheme or None

    origin = trusted_origin(_scheme)
    if origin is None:
        request_parts = urlsplit(request.host_url)
        origin = urlunsplit((_scheme or request_parts.scheme, request_parts.netloc, "", "", ""))

    return "{}{}".format(origin, url_for(endpoint, **values))
